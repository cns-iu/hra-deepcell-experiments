...
def load_data(path):
    ...
    df = pd.read_csv(path)
    return df[['Input_HuBMAP_ID']].dropna().reset_index(drop=True)
...
def main():
    ...
    # Configuration
    INPUT_PATH = "/u/sbdubey/CLI_HUBMAP/hra-deepcell-experiments/scripts/intestine-codex-stanford/data/deepcell_intestine.csv"
    OUTPUT_PATH = "/u/sbdubey/CLI_HUBMAP/hra-deepcell-experiments/scripts/intestine-codex-stanford/data/descendant_hubmapID_intestine_01.csv"
    ...
    for hubmap_id in tqdm(input_df['Input_HuBMAP_ID'], desc="Processing HuBMAP IDs"):
